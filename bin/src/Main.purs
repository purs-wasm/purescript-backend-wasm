  ...
  liftEffect (compileModules opts roots modules externs) >>= case _ of
    ...
    Right bytes -> do
      ...
      if args.text then do
        ...
        when (not (Array.null foreignMods)) (emitLoader bundleDir args.input foreignMods)
...
emitLoader :: FilePath -> FilePath -> Array String -> Aff Unit
emitLoader bundleDir input mods = do
  ...
  FS.writeTextFile UTF8 (Path.concat [ bundleDir, "index.mjs" ]) loaderSource
  Console.log (Fmt.fmt @"Wrote {file} (+ {n} foreign module(s))" { file: Path.concat [ bundleDir, "index.mjs" ], n: Array.length mods })

-- | A fixed, program-independent loader: it reads the imports out of the wasm and
-- | resolves each from `./foreign/<Module>.js`, so no per-build codegen is needed.
loaderSource :: String
loaderSource =
  """import { readFileSync } from "node:fs";
import { fileURLToPath } from "node:url";

// Generated by purescript-backend-wasm (ADR 0014). Instantiates the GC wasm,
// discovering its host imports and satisfying each from the matching foreign
// module's JS (copied alongside under ./foreign/<Module>.js).
...
for (const { module } of WebAssembly.Module.imports(mod)) {
  if (importObject[module]) continue;
  const ns = await import(new URL(`./foreign/${module}.js`, import.meta.url).href);
  importObject[module] = { ...ns };
}
