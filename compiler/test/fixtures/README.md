...
| `HostEff.corefn.json` | `HostEff.purs.sample` (+`.js`, +`.externs.cbor`) | `Test.E2E.HostEff` | **Host effectful FFI** (ADR 0015): real `foreign import record :: Int -> Effect Unit` / `log :: String -> Effect Unit` (JS `n => () => …`). Verifies externs `Effect`-result → `MEffect`, purity keeps the runs, `Perform` lowers to a host call, and the glue runs the thunk on the JS side. `runRec` ⇒ spy `[1,2]`; `greet` ⇒ a real `console.log "Hello, World!"`. Links the Effect/prelude set; needs externs (`instantiateForeignStr`) |
