...
# purs-backend-es / gnuplot on PATH); the `purs-wasm` CLI must be built (output/PursWasm.CLI.Main).
set -euo pipefail
...
node ./purs-wasm/index.js build -I ./bench/output -O ./bench/output-wasm-count-effect -e CountEffect
