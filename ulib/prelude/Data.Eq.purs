-- | ulib SHADOW of `prelude`'s `Data.Eq` (ADR 0028), targeting prelude 6.0.2.
-- |
-- | Byte-for-byte the upstream module EXCEPT `eqArrayImpl`, which here is PureScript over the
-- | WasmBase array primitives (`Wasm.Array`) with prelude-free loop arithmetic (`Wasm.Int`,
-- | since `Data.Eq` imports no `Prelude` arithmetic) — so the element comparator used by `Eq
-- | (Array a)` *specializes* (ADR 0027) instead of going through the opaque foreign
-- | `eqArrayImpl`. The remaining scalar foreigns (`eqIntImpl`/…) keep their wat providers. The
...
-- ulib shadow: `eqArrayImpl` over WasmBase primitives, so the element comparator specializes on
-- wasm (vs prelude's opaque `foreign import eqArrayImpl`). Lengths must match, then compare
-- elementwise, short-circuiting on the first inequality.
eqArrayImpl :: forall a. (a -> a -> Boolean) -> Array a -> Array a -> Boolean
eqArrayImpl f xs ys =
  if WI.eq nx ny then go 0 else false
  where
  nx = WA.length xs
  ny = WA.length ys
  go i =
    if WI.eq i nx then true
    else if f (WA.unsafeIndex xs i) (WA.unsafeIndex ys i) then go (WI.add i 1)
    else false
