-- | ulib SHADOW of `prelude`'s `Control.Apply` (ADR 0028), targeting prelude 6.0.2.
-- |
...
-- ulib shadow: `arrayApply` over WasmBase primitives, so the functions applied by `<*>` specialize
-- on wasm (vs prelude's opaque `foreign import arrayApply`). Nested loop: apply every function to
...
arrayApply fs xs = outer 0 0 (WA.unsafeNew (WI.mul l k))
  where
  l = WA.length fs
  k = WA.length xs
  outer i n out =
    if WI.eq i l then out
    else inner i 0 n out
  inner i j n out =
    if WI.eq j k then outer (WI.add i 1) n out
    else inner i (WI.add j 1) (WI.add n 1)
      (WA.unsafeSet out n ((WA.unsafeIndex fs i) (WA.unsafeIndex xs j)))
