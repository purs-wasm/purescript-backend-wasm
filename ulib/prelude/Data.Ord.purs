-- | ulib SHADOW of `prelude`'s `Data.Ord` (ADR 0028), targeting prelude 6.0.2.
-- |
-- | Byte-for-byte the upstream module EXCEPT `ordArrayImpl`, which here is PureScript over the
-- | WasmBase array primitives (`Wasm.Array`) with prelude-free loop arithmetic (`Wasm.Int`,
-- | since `Data.Ord`'s array loop needs no `Prelude` arithmetic) — so the element comparator used
-- | by `Ord (Array a)` *specializes* (ADR 0027) instead of going through the opaque foreign
-- | `ordArrayImpl`. The remaining scalar foreigns (`ordIntImpl`/…) keep their wat providers. The
-- | module's interface (exports, types, instances) is unchanged, so this can shadow the registry
-- | module at the user's resolved version.
module Data.Ord
  ( class Ord
  , compare
  , class Ord1
...
-- ulib shadow: `ordArrayImpl` over WasmBase primitives, so the element comparator specializes on
-- wasm (vs prelude's opaque `foreign import ordArrayImpl`). Walks both arrays in lockstep returning
-- the first non-zero element delta; if one array is a prefix of the other, the shorter compares
-- first (`1`) and the longer second (`-1`) — the codes prelude's `compare 0` maps to `LT`/`GT`.
ordArrayImpl :: forall a. (a -> a -> Int) -> Array a -> Array a -> Int
ordArrayImpl f xs ys = go 0
  where
  la = WA.length xs
  lb = WA.length ys
  go i =
    if WI.eq i la then (if WI.eq i lb then 0 else 1)
    else if WI.eq i lb then -1
    else
      let
        o = f (WA.unsafeIndex xs i) (WA.unsafeIndex ys i)
      in
        if WI.eq o 0 then go (WI.add i 1) else o
