-- | ulib SHADOW of `strings`' `Data.String.CodePoints` (ADR 0028 / 0030), targeting strings 6.0.1.
-- |
...
-- | `String` is UTF-8 and a `Char` is already a code point (ADR 0030), so `Data.String.CodeUnits`
...
-- A `CodePoint` and a `Char` are both an `Int` code point on this backend (ADR 0030); the
-- conversions are the identity on the underlying `Int` (`Wasm.Char`).
cpToChar :: CodePoint -> Char
cpToChar (CodePoint n) = WC.fromCodePoint n

charToCp :: Char -> CodePoint
charToCp c = CodePoint (WC.toCodePoint c)
