-- | ulib INTERNAL helper (ADR 0031 §6): the UTF-8 codec shared by the `Data.String.*` shadows. It is
...
-- | *injected* from the lib (the user never imports it), per ADR 0031 §6's plan→recompute→materialize
...
-- | byte-level `Wasm.String` primitives — no `Char`, so the dependency surface is just `Wasm.String`.
