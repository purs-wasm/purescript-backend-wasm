-- | ULIB SHADOW of `arrays`' `Data.Array` (ADR 0028), targeting arrays 7.3.x.
-- |
...
-- | PureScript over `Wasm.Array` primitives. On wasm those closures specialize (ADR 0027), so
...
-- | intrinsics on this backend, so no `Wasm.Int` is needed (unlike the sub-`Prelude` shadows).
...
findMap f xs = go 0
  ...
  go i =
    if i == n then Nothing
    else case f (WA.unsafeIndex xs i) of
      Nothing -> go (i + 1)
      m -> m
...
findIndex f xs = go 0
  ...
  go i =
    if i == n then Nothing
    else if f (WA.unsafeIndex xs i) then Just i
    else go (i + 1)
...
findLastIndex f xs = go (WA.length xs - 1)
  where
  go i =
    if i < 0 then Nothing
    else if f (WA.unsafeIndex xs i) then Just i
...
-- filter is not known up front, but we must allocate `Wasm.Array.unsafeNew` *once* and thread
...
filter f xs = go 0 0 (WA.unsafeNew (count 0 0))
  where
  n = WA.length xs
  count i acc = if i == n then acc else count (i + 1) (if f (WA.unsafeIndex xs i) then acc + 1 else acc)
  go i j out =
    if i == n then out
    else
      let x = WA.unsafeIndex xs i in
      if f x then go (i + 1) (j + 1) (WA.unsafeSet out j x)
      else go (i + 1) j out
...
scanl f z xs = go 0 z (WA.unsafeNew n)
  where
  n = WA.length xs
  go i acc out =
    if i == n then out
    else
      let acc' = f acc (WA.unsafeIndex xs i) in
      go (i + 1) acc' (WA.unsafeSet out i acc')
...
scanr f z xs = go (n - 1) z (WA.unsafeNew n)
  where
  n = WA.length xs
  go i acc out =
    if i < 0 then out
    else
      let acc' = f (WA.unsafeIndex xs i) acc in
      go (i - 1) acc' (WA.unsafeSet out i acc')
...
zipWith
  ...
  -> Array b
  -> Array c
zipWith f xs ys = go 0 (WA.unsafeNew m)
  where
  m = min (WA.length xs) (WA.length ys)
  go i out =
    if i == m then out
    else go (i + 1) (WA.unsafeSet out i (f (WA.unsafeIndex xs i) (WA.unsafeIndex ys i)))
...
any f xs = go 0
  ...
  go i =
    if i == n then false
    else if f (WA.unsafeIndex xs i) then true
    else go (i + 1)
...
all f xs = go 0
  ...
  go i =
    if i == n then true
    else if f (WA.unsafeIndex xs i) then go (i + 1)
    else false
