-- | ulib SHADOW of `foldable-traversable`'s `Data.Foldable` (ADR 0028), targeting 6.0.0.
-- | Byte-for-byte upstream EXCEPT `foldrArray` / `foldlArray`, which are PureScript over the
-- | WasmBase array primitives (`Wasm.Array` + prelude-free `Wasm.Int`) so the folded closure
...
-- ulib shadow: array folds over WasmBase primitives, so the folded closure specializes on wasm
-- (vs the opaque foreign). `foldl` ascends, `foldr` descends; both index in place (no build).
foldrArray :: forall a b. (a -> b -> b) -> b -> Array a -> b
foldrArray f z xs = go (WA.length xs) z
  where
  go i acc = if WI.eq i 0 then acc else let j = WI.sub i 1 in go j (f (WA.unsafeIndex xs j) acc)

foldlArray :: forall a b. (b -> a -> b) -> b -> Array a -> b
foldlArray f z xs = go 0 z
  where
  n = WA.length xs
  go i acc = if WI.eq i n then acc else go (WI.add i 1) (f acc (WA.unsafeIndex xs i))
