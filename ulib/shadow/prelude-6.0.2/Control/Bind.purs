-- | ulib SHADOW of `prelude`'s `Control.Bind` (ADR 0028), targeting prelude 6.0.2.
-- |
...
-- ulib shadow: `arrayBind` (concatMap) over WasmBase primitives, so the bound closure specializes
-- on wasm (vs prelude's opaque `foreign import arrayBind`). Two passes over precomputed sub-arrays:
...
arrayBind xs f = fillOuter 0 0 (WA.unsafeNew total)
  where
  n = WA.length xs
  subs = mkSubs 0 (WA.unsafeNew n)
  mkSubs i acc =
    if WI.eq i n then acc
    else mkSubs (WI.add i 1) (WA.unsafeSet acc i (f (WA.unsafeIndex xs i)))
  total = sumLen 0 0
  sumLen i acc =
    if WI.eq i n then acc
    else sumLen (WI.add i 1) (WI.add acc (WA.length (WA.unsafeIndex subs i)))
  fillOuter i o out =
    if WI.eq i n then out
    else fillInner i 0 o out
  fillInner i j o out =
    if WI.eq j (WA.length (WA.unsafeIndex subs i)) then fillOuter (WI.add i 1) o out
    else fillInner i (WI.add j 1) (WI.add o 1)
      (WA.unsafeSet out o (WA.unsafeIndex (WA.unsafeIndex subs i) j))
